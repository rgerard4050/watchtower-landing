...
const { createCheckout, requireCheckoutReadiness, resolveOrigin } = require('../server/stripe-payments');
...
module.exports = async function checkoutHandler(req, res) {
  ...
  try {
    const { payment_mode: mode } = requireCheckoutReadiness(process.env);
